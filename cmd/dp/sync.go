...
func init() {
	...
	syncCommand.PersistentFlags().Bool("gh-annotate", false, "Output suitable for github workflow annotation (ignores -s)")
	syncCommand.PersistentFlags().BoolP("dry-run", "d", false, "Only print planned changes without performing them")
	syncCommand.PersistentFlags().IntP("concurrency", "c", 3, "The number of validation requests to perform at once (maximum 10)")
}
